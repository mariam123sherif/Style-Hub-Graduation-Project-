...
// ─── BRAND CONFIG — change anything here to customise this page ───
const BRAND = {
  name:        "Ninos",
...
// ─── SIZE RATIOS — one place to resize every section ───
const SZ = {
  heroBanner:    420,    // px  — hero height
  heroLogoSize:  200,    // px  — logo circle diameter
  photoRowH:     280,    // px  — 3-photo strip height
  popularCardH:  240,    // px  — popular mini-card height
  shopCardH:     220,    // px  — grey-section card height
  gridCardRatio: "4/4",  // css aspect-ratio for product grid cards
};

// ─── DATA ───
const NINOS    = PRODUCTS.filter(p => p.brand.toUpperCase() === "NINOS");
const ALL_SIZES  = [...new Set(NINOS.flatMap(p => p.sizes))].sort((a,b)=>parseInt(a)-parseInt(b));
const ALL_COLORS = [...new Set(NINOS.flatMap(p => p.colors))];
  ...
  return (
    <div onClick={() => navigate(`/product/${p.id}`)}
      ...
      <div style={{ padding:".55rem .65rem" }}>
        <div style={{ fontSize:".52rem", letterSpacing:".15em", textTransform:"uppercase", color:"var(--warm)", marginBottom:".15rem" }}>{p.category||"kids"}</div>
...
<section style={{
        ...
        <img src={BRAND.heroBg} alt="" aria-hidden
          style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:.12,borderRadius:10}}
          onError={e=>e.target.style.display="none"}/>
        ...
        <div style={{position:"relative",zIndex:2,display:"flex",alignItems:"center",gap:"4rem",padding:"0 6%",width:"100%"}}>
          ...
          {/* LOGO */}
          <div style={{flexShrink:0,width:SZ.heroLogoSize,height:SZ.heroLogoSize,borderRadius:"50%",overflow:"hidden",background:"#fff",border:"1.5px solid rgba(26,26,24,.12)",boxShadow:"0 8px 32px rgba(26,26,24,.12)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          ...
          <div style={{maxWidth:500}}>
            ...
            <p style={{fontSize:".90rem",lineHeight:1.85,color:"#555252ff",marginBottom:"1.8rem",maxWidth:440}}>
              {BRAND.desc}
            </p>
            <button onClick={scrollGrid} className="ninos-cta-btn">Shop Now</button>
          </div>
        </div>
      </section>
      ...
      <section style={{background:"#D8D4CE",padding:"4rem 6%",borderTop:"1px solid rgba(26,26,24,.08)",borderBottom:"1px solid rgba(26,26,24,.08)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"1rem"}}>
          {NINOS.slice(4,8).map((p,i)=>(
            <div key={p.id} className={`reveal d${i+1}`} ref={addRef} style={{cursor:"pointer",textAlign:"center"}}
              ...
              <div style={{fontSize:".79rem",fontWeight:500,marginBottom:".18rem"}}>{p.name}</div>
              <div style={{fontSize:".70rem",color:"var(--warm)"}}>{p.price}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ════════════════════════════════
          ...
          text (left) | black blob (right)
      ════════════════════════════════ */}
      <section style={{background:"#ffffffff",padding:"5rem 6%",borderBottom:"2px solid rgba(26,26,24,.12)"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5rem",alignItems:"center"}}>
          ...
          <div className="reveal" ref={addRef}>
            ...
            <p style={{fontSize:".86rem",lineHeight:1.85,color:"#555252ff",marginBottom:"2rem",maxWidth:400}}>{BRAND.desc}</p>
            <button onClick={scrollGrid} className="ninos-cta-btn">Click Here</button>
          </div>

          {/* RIGHT — decorative black blob */}
      ...
      <div style ={{display:"flex",gap:"2.5rem",padding:"3rem 6%",alignItems:"flex-start",background:"var(--cream)"}}>
        ...
        <div style={{width:185,flexShrink:0,position:"sticky",top:"70px"}}>
          ...
          {(selSizes.length>0||selColors.length>0||selCategory!=="all"||selType!=="all")&&(
            <button onClick={()=>{setSelSizes([]);setSelColors([]);setSelCategory("all");setSelType("all");setPage(1);}}
