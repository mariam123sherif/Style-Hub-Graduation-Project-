...
function SellerSignInForm({ onForgot, onSwitchSignUp, onDone }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    setErr("");
    if (!email || !isEmail(email)) { setErr("Please enter a valid email."); return; }
    if (!pw) { setErr("Please enter your password."); return; }
    setBusy(true);
    setTimeout(() => { setBusy(false); setOk(true); setTimeout(onDone, 900); }, 1300);
  };

  return (
    <form onSubmit={submit} className="fade-up">
      {ok && <div className="sh-alert sh-alert-success"><i className="fas fa-check-circle" style={{marginRight:8}} />Welcome back! Redirecting to dashboard…</div>}
      {err && <div className="sh-alert sh-alert-error"><i className="fas fa-exclamation-circle" style={{marginRight:8}} />{err}</div>}
      <label className="sh-label">Email address</label>
      <input className="sh-input" type="email" placeholder="seller@example.com" value={email} onChange={e => { setEmail(e.target.value); setErr(""); }} />
      <label className="sh-label">Password</label>
      <div className="sh-input-wrap sh-mb">
        <input className="sh-input" type={show ? "text" : "password"} style={{paddingRight:46}} placeholder="••••••••" value={pw} onChange={e => { setPw(e.target.value); setErr(""); }} />
        <button type="button" className="sh-eye" onClick={() => setShow(v => !v)}><i className={`far ${show ? "fa-eye-slash" : "fa-eye"}`} /></button>
      </div>
      ...
      <button className="sh-btn" type="submit" disabled={busy}>
        {busy ? <><i className="fas fa-circle-notch fa-spin" /> Signing in…</> : <><i className="fas fa-store" /> ACCESS MY SELLER DASHBOARD</>}
      </button>
      ...
      <div className="sh-social">
        ...
        <button type="button" className="sh-social-btn">
          <i className="fab fa-facebook-f" style={{color:"#1877f2"}} /> Facebook
        </button>
      </div>
      <p className="sh-switch">New seller? <button type="button" onClick={onSwitchSignUp}>Create a seller account</button></p>
    </form>
  );
}
...
function SellerSignUpForm({ onSwitchSignIn, onDone }) {
  ...
  const [form, setForm] = useState({
    ...
    storeName:"", category:"", shopType:"fashion", bio:""
  });
  const [show, setShow] = useState(false);
  const [err, setErr] = useState("");
  const [ok, setOk] = useState(false);
  ...
  const next = (e) => {
    ...
    if (step === 1) {
      if (!form.first) { setErr("First name required."); return; }
      if (!form.last)  { setErr("Last name required."); return; }
      if (!isEmail(form.email)) { setErr("Valid email required."); return; }
      if (form.pw.length < 6) { setErr("Password must be at least 6 characters."); return; }
      if (form.pw !== form.cpw) { setErr("Passwords don't match."); return; }
      setStep(2);
    } else if (step === 2) {
      if (!form.storeName.trim()) { setErr("Store name required."); return; }
      if (!form.category) { setErr("Please select a category."); return; }
      if (!agreed) { setErr("Please accept the seller terms."); return; }
      setBusy(true);
      setTimeout(() => { setBusy(false); setOk(true); setTimeout(onDone, 1100); }, 1500);
    }
  };
  ...
  return (
    <form onSubmit={next} className="fade-up">
      ...
      {ok && <div className="sh-alert sh-alert-success"><i className="fas fa-check-circle" style={{marginRight:8}} />Store created! Redirecting to dashboard…</div>}
      {err && <div className="sh-alert sh-alert-error"><i className="fas fa-exclamation-circle" style={{marginRight:8}} />{err}</div>}

      {step === 1 && (
        <>
          <p style={{fontSize:".75rem",fontWeight:700,color:"var(--green)",letterSpacing:".8px",textTransform:"uppercase",marginBottom:".5rem"}}>Step 1 of 2 · Account Info</p>
          <div className="sh-name-row">
            <div>
              <label className="sh-label">First Name</label>
              <input className="sh-input" type="text" placeholder="Sara" value={form.first} onChange={set("first")} />
            </div>
            <div>
              <label className="sh-label">Last Name</label>
              <input className="sh-input" type="text" placeholder="Ahmed" value={form.last} onChange={set("last")} />
            </div>
          </div>
          <label className="sh-label">Email address</label>
          <input className="sh-input" type="email" placeholder="sara@example.com" value={form.email} onChange={set("email")} />
          <label className="sh-label">Password</label>
          <div className="sh-input-wrap sh-mb">
            <input className="sh-input" type={show ? "text" : "password"} style={{paddingRight:46}} placeholder="••••••••" value={form.pw} onChange={set("pw")} />
            <button type="button" className="sh-eye" onClick={() => setShow(v => !v)}><i className={`far ${show ? "fa-eye-slash" : "fa-eye"}`} /></button>
          </div>
          {form.pw && (
          ...
          <input className="sh-input" type="password" placeholder="••••••••" value={form.cpw} onChange={set("cpw")} />
          <button className="sh-btn" type="submit"><i className="fas fa-arrow-right" /> CONTINUE</button>
        </>
      )}

      {step === 2 && (
        <>
          <p style={{fontSize:".75rem",fontWeight:700,color:"var(--green)",letterSpacing:".8px",textTransform:"uppercase",marginBottom:".5rem"}}>Step 2 of 2 · Store Details</p>
          <label className="sh-label">Store Name</label>
          <input className="sh-input" type="text" placeholder="e.g. Sara's Closet" value={form.storeName} onChange={set("storeName")} />
          <label className="sh-label" style={{marginBottom:".6rem"}}>What will you sell?</label>
          <div className="shop-types" style={{marginBottom:"1rem"}}>
            {SHOP_TYPES.map(t => (
              <button key={t.id} type="button" className={`shop-type-btn${form.shopType === t.id ? " selected" : ""}`} onClick={() => setForm(f => ({...f, shopType: t.id}))}>
                <i className={t.icon} /><span>{t.label}</span>
              </button>
            ))}
          </div>
          <label className="sh-label">Main Category</label>
          <select className="sh-select" value={form.category} onChange={set("category")}>
          ...
          <textarea className="sh-textarea" placeholder="Tell customers about your brand…" value={form.bio} onChange={set("bio")} />
          <div className="sh-check-row">
            <input type="checkbox" id="agree" checked={agreed} onChange={e => setAgreed(e.target.checked)} />
            <label htmlFor="agree">I agree to StyleHub's <a href="#">Seller Terms</a> and <a href="#">Commission Policy</a></label>
          </div>
          <button className="sh-btn" type="submit" disabled={busy}>
            {busy ? <><i className="fas fa-circle-notch fa-spin" /> Creating Store…</> : <><i className="fas fa-store" /> LAUNCH MY STORE</>}
          </button>
          <button type="button" onClick={() => setStep(1)} style={{width:"100%",marginTop:".7rem",background:"none",border:"none",cursor:"pointer",fontFamily:"var(--font)",fontSize:".8rem",color:"var(--gray-text)",textDecoration:"underline"}}>
            ← Back
          </button>
        </>
      )}

      <p className="sh-switch">Already a seller? <button type="button" onClick={onSwitchSignIn}>Sign in</button></p>
    </form>
  );
}
...
/* ─────── Benefits Panel ─────── */
const BENEFITS = [
  { icon: "fas fa-chart-line", title: "Reach Thousands of Buyers", desc: "StyleHub connects you with a curated audience of fashion-forward shoppers actively looking to buy." },
  { icon: "fas fa-percent", title: "Low Commission, High Returns", desc: "Keep more of every sale with our competitive seller fees — among the lowest in Egyptian fashion e-commerce." },
  { icon: "fas fa-box-open", title: "Easy Product Management", desc: "Upload, manage, and update your inventory through an intuitive seller dashboard built for speed." },
  { icon: "fas fa-headset", title: "Dedicated Seller Support", desc: "Our team is always ready to help you grow — from onboarding to after-sale queries." },
];

function BenefitsPanel() {
  return (
    <div className="benefits-panel">
      <h2>Start <span>Selling</span> on StyleHub Today</h2>
      <p>Join hundreds of Egyptian fashion brands and independent sellers already growing their business on StyleHub.</p>
      ...
      <div className="testimonial-box">
        <p>"StyleHub gave my brand the visibility I needed. Within 3 months we doubled our monthly sales — the dashboard is incredibly easy to use."</p>
...
export default function SellerAuthPage({ onSellerLoggedIn }) {
  ...
  const handleDone = () => { if (onSellerLoggedIn) onSellerLoggedIn(); };
      ...
      <div className="seller-page">
        ...
        <div className="seller-main">
          ...
          <div className="seller-auth-card">
            ...
            {mode === "signin"
              ? <SellerSignInForm onForgot={() => {}} onSwitchSignUp={() => go("signup")} onDone={handleDone} />
              : <SellerSignUpForm onSwitchSignIn={() => go("signin")} onDone={handleDone} />
            }
          </div>
        </div>
